...
- **Login Notifications** (Nov 2025): Security feature that sends email alerts when users successfully log into their account. Notifications include comprehensive login details (date/time, location, device, browser, OS, IP address) with professional email templates. Users can enable/disable this feature in Security Settings via the `user_security_settings.loginalerts` flag. Implemented in AuthContext with non-blocking email delivery to ensure login performance.
