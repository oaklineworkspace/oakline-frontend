...
const parseUserAgent = (userAgent) => {
  ...
  if (/(tablet|ipad|playbook|silk)|(android(?!.*mobi))/i.test(userAgent)) {
    ...
    if (/ipad/i.test(userAgent)) {
      deviceModel = 'iPad';
      // Try to detect iPad model
      if (/ipad.*OS (\d+)_/i.test(userAgent)) {
        const match = userAgent.match(/ipad.*OS (\d+)_/i);
    ...
    // Detect iPhone with more detail
    if (/iphone/i.test(userAgent)) {
      deviceModel = 'iPhone';
      // Try to extract iOS version for better identification
      if (/iPhone OS (\d+)_/i.test(userAgent)) {
        const match = userAgent.match(/iPhone OS (\d+)_/i);
        deviceModel = `iPhone (iOS ${match[1]})`;
      }
      // Try to detect specific iPhone models based on screen size or other indicators
      if (/iPhone.*CPU.*OS 1[6-9]|iPhone.*CPU.*OS [2-9]\d/.test(userAgent)) {
        // Modern iPhone (iOS 16+)
        deviceModel = deviceModel.replace('iPhone', 'iPhone (Modern)');
      }
    } 
    // Detect iPod
