...
export async function sendEmail({ to, subject, text, html, from }) {
  try {
    ...
    // Use the provided 'from' parameter if available, otherwise fallback to default
...
export async function sendLoanNotificationEmail({ to, loanAmount, loanType, status, remarks }) {
    ...
    <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif; background-color: #f8fafc;">
      <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
        ...
        <div style="background: linear-gradient(135deg, ${isApproved ? '#059669 0%, #10b981 100%' : '#dc2626 0%, #ef4444 100%'}); padding: 32px 24px; text-align: center;">
          <div style="color: #ffffff; font-size: 32px; margin-bottom: 8px;">🏦</div>
          <div style="color: #ffffff; font-size: 28px; font-weight: 700; margin-bottom: 8px;">Oakline Bank</div>
          <div style="color: #ffffff; opacity: 0.9; font-size: 16px;">Loan Application Update</div>
        </div>

        <!-- Main Content -->
        <div style="padding: 40px 32px;">
          <h1 style="color: #1a365d; font-size: 28px; font-weight: 700; margin: 0 0 16px 0;">
            ${isApproved ? '✅ Loan Application Approved!' : '❌ Loan Application Update'}
          </h1>

          <p style="color: #4a5568; font-size: 16px; line-height: 1.6; margin: 0 0 24px 0;">
            ${isApproved 
              ? 'Great news! Your loan application has been reviewed and approved.' 
              : 'We have reviewed your loan application. Unfortunately, we are unable to approve your request at this time.'}
          </p>
          ...
          <div style="background-color: #f7fafc; border-radius: 12px; padding: 24px; margin: 24px 0;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr>
                ...
                <td style="padding: 8px 0; color: #1a365d; font-size: 14px; font-weight: 600; text-align: right;">$${parseFloat(loanAmount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
              </tr>
              <tr>
          ...
          <div style="background-color: ${isApproved ? '#f0fdf4' : '#fef2f2'}; border-left: 4px solid ${isApproved ? '#10b981' : '#dc2626'}; padding: 16px; margin: 24px 0;">
            <p style="color: #1a365d; font-size: 14px; font-weight: 600; margin: 0 0 8px 0;">
              ${isApproved ? 'Additional Information:' : 'Reason for Decline:'}
            </p>
          ...
          <div style="background-color: #eff6ff; border-radius: 12px; padding: 20px; margin: 24px 0;">
            ...
            <ul style="color: #4a5568; font-size: 14px; line-height: 1.8; margin: 0; padding-left: 20px;">
              <li style="margin-bottom: 8px;">Your funds will be disbursed to your account within 1-2 business days</li>
              <li style="margin-bottom: 8px;">You will receive a separate email with your loan agreement and repayment schedule</li>
              <li style="margin-bottom: 8px;">Log in to your account to view detailed loan information</li>
              <li>Contact us if you have any questions about your loan</li>
            </ul>
          </div>
          ` : `
          <!-- Reapply Information -->
          <div style="background-color: #eff6ff; border-radius: 12px; padding: 20px; margin: 24px 0;">
            ...
            <ul style="color: #4a5568; font-size: 14px; line-height: 1.8; margin: 0; padding-left: 20px;">
              <li style="margin-bottom: 8px;">Review the reason for decline and work on improving those areas</li>
              <li style="margin-bottom: 8px;">You may reapply for a loan in the future</li>
              <li style="margin-bottom: 8px;">Contact our loan specialists for personalized advice</li>
              <li>Consider alternative loan products that may better fit your situation</li>
            </ul>
          </div>
          `}
          ...
          <!-- CTA Button -->
          <div style="text-align: center; margin: 32px 0;">
            <a href="${process.env.NEXT_PUBLIC_BASE_URL || 'https://your-domain.com'}/loan/dashboard" 
               style="display: inline-block; background: linear-gradient(135deg, #0066cc 0%, #2c5aa0 100%); 
                      color: #ffffff; padding: 16px 32px; border-radius: 12px; text-decoration: none; 
                      font-weight: 600; font-size: 16px; box-shadow: 0 4px 12px rgba(0, 102, 204, 0.3);">
              View Loan Dashboard
  ...
  return sendEmail({
    to,
    subject: `${isApproved ? '✅' : '❌'} Oakline Bank Loan Application ${status}`,
    html: emailHtml,
    from: process.env.SMTP_FROM_NOTIFY || process.env.SMTP_FROM
  });
}