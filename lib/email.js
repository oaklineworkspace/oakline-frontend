...
export async function sendTransferSentEmail({ to, userName, recipientName, amount, reference, accountNumber, userId = null }) {
  const bankDetails = await getBankDetails();
        ...
        <div style="padding: 32px;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
            ...
            <tr style="border-bottom: 1px solid #e2e8f0;">
              ...
              <td style="padding: 12px 0; color: #1a365d; font-size: 14px; font-weight: 600; text-align: right;">${new Date().toLocaleString('en-US', {
                year: 'numeric',
                month: 'short',
                day: 'numeric',
                hour: '2-digit',
...
export async function sendTransferReceivedEmail({ to, userName, senderName, amount, reference, accountNumber, userId = null }) {
  const bankDetails = await getBankDetails();
  const bankName = bankDetails?.name || 'Oakline Bank';
  const supportEmail = bankDetails?.email_support || bankDetails?.email_contact || 'contact-us@theoaklinebank.com';
  const supportPhone = bankDetails?.phone || '(636) 635-6122';
