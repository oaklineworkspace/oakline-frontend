...
const QUEUE_DELAY = 50; // Minimal delay for faster translations (50ms between requests)
...
async function processQueue() {
  ...
  while (requestQueue.length > 0) {
    const { text, targetLanguage, sourceLanguage, resolve, reject } = requestQueue.shift();
...
export async function translateText(text, targetLanguage, sourceLanguage = 'en') {
  ...
  // Add to queue and process with timeout fallback
  return new Promise((resolve) => {
    // Set a longer timeout to prevent premature timeouts
    const timeoutId = setTimeout(() => {
      console.warn('Translation timeout, returning original text');
      // Remove from queue if still there
      const index = requestQueue.findIndex(q => q.text === text && q.targetLanguage === targetLanguage);
      if (index > -1) {
    ...
    requestQueue.push({ 
      ...
      reject: (error) => {
        clearTimeout(timeoutId);
        console.error('Translation rejected:', error);
        resolve(text);
      }
    });
